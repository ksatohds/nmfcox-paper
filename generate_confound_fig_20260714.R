# 【20260714】fig:confound を (B) 版 mc_conf_conf500B58.rds から再描画（PNG＋PDF）。フォント1.5倍（pointsize 12→18）。
# 【20260713B】(B) 版 mc_conf_conf500B58.rds（500reps, 全4設定）から再描画。
# Working directory = repository root (all paths below are relative to it). If needed: setwd("/path/to/nmfcox-paper")
GAMMA0<-0.5
res <- readRDS("verification/mc_conf_conf500B58.rds")
ord <- c("n300_rho.0_cA3","n300_rho.5_cA3","n300_rho.7_cA3","n600_rho.5_cA3"); ord<-ord[ord %in% names(res)]
est  <- c("cox_z","full","nmf","tvc","oracle")
elab <- c("(1) Cox (z-only)","(2) full-PH Cox","(3) NMF-COX (CF)","tvc-Cox (splines)","oracle")
...
draw <- function(){
  ...
  ecol<-c("gray45","black","black","gray30","gray25"); epch<-c(1,0,19,5,4)
  elwd<-c(1.6,1.6,2.2,1.7,1.8); ecex<-c(1.15,1.15,1.3,1.2,1.2); yb<-length(est):1
...
png("v3_fig/confound_forest.png",width=2300,height=860,res=175,type=ptype,pointsize=18); draw(); dev.off()
grDevices::cairo_pdf("v3_fig/confound_forest.pdf", width=2300/175, height=860/175, pointsize=18); draw(); dev.off()
