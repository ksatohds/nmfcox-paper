# 【20260724 監査】必須-1: 固定 λ_X では平滑化バイアス下限が残り、nuisance RMSE は N^-0.09 でしか減らない
#   （表3 実測: RMSE*N^{1/4} が 0.479->0.671 と増加）。ここでは λ_X ∝ N^{-1}（300/150/75/37.5）と縮小させ、
#   o_p(N^{-1/4}) を満たしうる領域が存在するかを実測する。他は mc_beta_20260712.R と同一。
...
SETTINGS <- list(
  `1`=list(n=150,  betaA=0.4, rank=1, SM=300.0, tag="n150_rate"),
  `2`=list(n=300,  betaA=0.4, rank=1, SM=150.0, tag="n300_rate"),
  `3`=list(n=600,  betaA=0.4, rank=1, SM= 75.0, tag="n600_rate"),
  `4`=list(n=1200, betaA=0.4, rank=1, SM= 37.5, tag="n1200_rate"))
...
one_rep <- function(r, S){
  ...
  fsI<-tryCatch(nmf.cox(y~z,data=d,A=~a,rank=S$rank,X.L2.smooth=S$SM,ties="breslow",
