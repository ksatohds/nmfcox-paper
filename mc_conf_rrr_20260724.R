# 【20260724 監査】RRR(reduced-rank, coxvcf)を交絡シミュに第6の推定量として追加し、真値既知の設定で
#   gamma のバイアス/被覆を測る。狙い: 「RRR の beta(t) は振動が大きいが、PH変数の効果 gamma は
#   本当に不偏に推定できているのか」を直接検証する（実データの『ランクに対する安定性』は不偏性の証拠にならない）。
#   R=1（a はスカラー）なので RRR の縮約ランクは r=1、辞書は定数+自然スプライン(df=3) の M=4。
...
source("nmf_cox2.R"); source("nmf_cox3.R")
source("coxvc.r"); source("coxvcf.r")   # RRR 比較用（Perperoglou; 時間昇順が前提）
...
OFFS  <- if(length(args)>=5) as.integer(args[5]) else 0L   # seed オフセット（2台分割）
progf <- sprintf("verification/mc_conf_rrr_progress_%s.txt", outtag)
...
one_rep <- function(r, S){
  ...
  out<-c(cox_z=NA,se0=NA,full=NA,se1=NA,nmf=NA,se3=NA,tvc=NA,se_tvc=NA,rrr=NA,se_rrr=NA,oracle=NA,evt=mean(d$status))
  ...
  cf<-tryCatch(nmf.cox3.cf(y~z,data=d,A=~a,rank=S$rank,X.smooth=S$SM,nfolds=NFOLD,seed=r),error=function(e)NULL)
  ...
  ## RRR (Perperoglou): 供給辞書 M=4 (定数+ns(df=3)) を縮約ランク r=1 に。z は固定効果として同時推定。
  ##  重要: coxvcf の内部 (full.fitf) はリスク集合を X[i:n,] として扱うため **データは時間昇順が前提**。
  ##  係数ベクトルは c(theta(p*q), fixed(v)) の順なので、z の推定値/SE は末尾の v 要素。
  fR<-tryCatch({ ds<-d[order(d$time),]; rownames(ds)<-NULL
                 Xr<-matrix(ds$a,ncol=1); colnames(Xr)<-"a"
                 Ftr<-cbind(1, ns(ds$time, df=3))
                 coxvcf(Surv(time,status)~z, X=Xr, Ft=Ftr, rank=1, data=ds) },
               error=function(e)NULL)
  if(!is.null(fR)){
    out["rrr"]   <- tryCatch(as.numeric(fR$f.coef)[length(as.numeric(fR$f.coef))], error=function(e)NA)
    se_all       <- tryCatch(as.numeric(fR$ster), error=function(e)NA)
    out["se_rrr"]<- if(length(se_all)>=1) se_all[length(se_all)] else NA }
...
for(si in sets){ S<-SETTINGS[[as.character(si)]]; t0<-Sys.time()
  ...
  mm<-mclapply(seq_len(Rrep), function(i){ r<-OFFS+i; v<-tryCatch(one_rep(r,S),error=function(e)rep(NA,12))
    if(r%%25==0) cat(sprintf("[%s] setting %d: %d/%d\n",format(Sys.time(),"%H:%M:%S"),si,r,Rrep),file=progf,append=TRUE); v }, mc.cores=cores)
  M<-do.call(rbind,mm); colnames(M)<-c("cox_z","se0","full","se1","nmf","se3","tvc","se_tvc","rrr","se_rrr","oracle","evt")
...
saveRDS(results, sprintf("verification/mc_conf_rrr_%s.rds", outtag))
